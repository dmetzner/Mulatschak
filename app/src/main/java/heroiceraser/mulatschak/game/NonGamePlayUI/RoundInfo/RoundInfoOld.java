...
//  Round Info Class
//
public class RoundInfoOld extends DrawableObject {

    private TextField new_round_;
    private TextField mulatschak_;
    ...
    public void init(GameView view) {
        ...
        mulatschak_.init(view, "", monitor_size_.x, Color.WHITE);
    ...
    public void updateMulatschak(GameView view) {
        ...
        for (int i = 0; i < view.getController().getAmountOfPlayers(); i++) {
            ...
            else if (view.getController().getPlayerById(i).getTricksToMake() != MakeBidsAnimation.MULATSCHAK) {
                text += "macht keinen Mulatschak";
            }
            else {
                text += "will einen Mulatschak machen!";
            }
            if (i != view.getController().getAmountOfPlayers() - 1) {
                text += "\n";
            }
        }
        mulatschak_.update(text, monitor_size_.y);
    }
    ...
    public void updateRoundInfo(GameController controller) {
        ...
        if (controller.getLogic().getTricksToMake() == MakeBidsAnimation.MULATSCHAK) {
            text_ += " von 5 Stiche\n";
        }
        else {
            text_ += " von " + controller.getLogic().getTricksToMake() + " Stiche\n";
        }
    ...
    */
    ...
    public TextField getMulatschak() {
        return mulatschak_;
    }

    public void setInfoBoxEmpty() {
        new_round_.setVisible(false);
        mulatschak_.setVisible(false);
